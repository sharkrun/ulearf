...
class Network(IP):
    ...
    def host_index(self, index):
        '''
        index available host in this subnet.
        '''
        if (not isinstance(index, int)) or (index > self.size()):
            raise ValueError('%r: index out of range' % (index,))

        return IP(self.network_long() + index, version=self.version())
