...
const (
	...
	rootCertBundle   = "/root/server.crt"
	licenseIssuer    = "youruncloud"
	acceptedAudience = "Every One"
)
...
type ClaimSet struct {
	...
	// Private claims
	Access *ResourceLimit `json:"access"`
}
...
func NewToken(rawToken string) (*Token, error) {
	...
	defer func() {
		if err != nil {
			log.Info("error while unmarshalling raw token: %s", err)
		}
	}()
...
func (t *Token) Verify(verifyOpts VerifyOptions) error {
	...
	if !contains(verifyOpts.TrustedIssuers, t.Claims.Issuer) {
		log.Info("token from untrusted issuer: %q", t.Claims.Issuer)
		return ErrInvalidToken
	}

	// Verify that the Audience claim is allowed.
	if !contains(verifyOpts.AcceptedAudiences, t.Claims.Audience) {
		log.Info("token intended for another audience: %q", t.Claims.Audience)
		return ErrInvalidToken
	}
	...
	if currentTime.After(ExpWithLeeway) {
		log.Info("token not to be used after %s - currently %s", ExpWithLeeway, currentTime)
		return ErrInvalidToken
	}

	NotBeforeWithLeeway := time.Unix(t.Claims.NotBefore, 0).Add(-Leeway)
	if currentTime.Before(NotBeforeWithLeeway) {
		log.Info("token not to be used before %s - currently %s", NotBeforeWithLeeway, currentTime)
		return ErrInvalidToken
	}
	...
	if err := signingKey.Verify(strings.NewReader(t.Raw), t.Header.SigningAlg, t.Signature); err != nil {
		log.Info("unable to verify token signature: %s", err)
		return ErrInvalidToken
	}
...
func LoadPublicKey() (*VerifyOptions, error) {
	...
	return &VerifyOptions{
		TrustedIssuers:    []string{licenseIssuer},
		AcceptedAudiences: []string{acceptedAudience},
		Roots:             rootPool,
